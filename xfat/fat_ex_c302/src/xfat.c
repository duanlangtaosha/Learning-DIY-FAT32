 ...
 * @return
 */
static xfat_err_t create_dbr (dbr_t * dbr, xdisk_part_t * xdisk_part, xfat_fmt_ctrl_t * ctrl) {
    xfat_err_t err;
    ...
    err = xdisk_write_sector(disk, (u8_t *)dbr, xdisk_part->start_sector, 1);
    if (err < 0) {
        return err;
    }

    // 同时在备份区中写一个备份
    err = xdisk_write_sector(disk, (u8_t *)dbr, xdisk_part->start_sector + dbr->fat32.BPB_BkBootSec, 1);
    if (err < 0) {
        return err;
    }
...
static xfat_err_t create_root_dir(xfat_fmt_info_t * fmt_info, xdisk_part_t * xdisk_part, xfat_fmt_ctrl_t * ctrl) {
    ...
    // 擦除根目录所在的簇
    memset(temp_buffer, 0, xdisk->sector_size);
    sector = xdisk_part->start_sector + data_sector;
    for (i = 0; i < fmt_info->sec_per_cluster; i++) {
        err = xdisk_write_sector(xdisk, temp_buffer, sector + i, 1);
        if (err < 0) {
            return err;
        }
    }
    ...
    if (ctrl->vol_name) {
        ...
        err = xdisk_write_sector(xdisk_part->disk, temp_buffer, sector, 1);
        if (err < 0) {
            return err;
        }
    }
...
static xfat_err_t create_fsinfo(xfat_fmt_info_t * fmt_info, xdisk_part_t * xdisk_part, xfat_fmt_ctrl_t * ctrl) {
    xfat_err_t err;
    fsinto_t * fsinfo = (fsinto_t *)temp_buffer;
    xdisk_t * disk = xdisk_part->disk;
...
xfat_err_t xfat_format (xdisk_part_t * xdisk_part, xfat_fmt_ctrl_t * ctrl) {
    ...
    // 创建dbr头
    err = create_dbr(dbr, xdisk_part, ctrl);
    if (err < 0) {
        return err;
    }
